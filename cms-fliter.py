    ...
    try:
        ...
        if path4.status_code == 200:
            if 'WordPress' in path4.content.encode('utf-8'):
                print(ktn3yell + '[WORDPRESS] -->   ' + '(' + url + ')' + CEND)
...
def laravel(url):
    try:
        path0 = se.get(url, headers=Agent, verify=False, timeout=50)
        path9 = se.get(url + '.env', headers=Agent, verify=False, timeout=50)
        if path9.status_code == 200:
            if 'Laravel' in path9.content.encode('utf-8'):
                print(ktn3yell + '[LARAVEL] -->   ' + '(' + url + ')' + CEND)
    ...
    try:
        ...
        if path1.status_code == 200:
            if 'Joomla' in path1.content.encode('utf-8'):
                print(ktngreen + '[Joomla] -->   ' + '(' + url + ')' + CEND)
    ...
    try:
        ...
        if path6.status_code == 200 :
            if 'Drupal' in path6.content.encode('utf-8'):
                print(ktn4blue + '[Drupal] -->   ' + '(' + url + ')' + CEND)
...
def prestashop(url):
    try:
        ...
        if 'content="PrestaShop"' in path0.content.encode('utf-8'):
            print(ktn5purp + '[PrestaShop] -->   ' + '(' + url + ')' + CEND)
...
def magento(url):
    try:
        ...
        if 'magento' in path0.content.encode('utf-8') or 'Magento' in path0.content.encode('utf-8'):
            print(ktn7grey + '[Magento] -->   ' + '(' + url + ')' + CEND)
...
def opencart(url):
    try:
        ...
        if 'catalog/view/' in path0.content.encode('utf-8'):
            print(ktn6blueblue + '[Opencart] -->   ' + '(' + url + ')' + CEND)
